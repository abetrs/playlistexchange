...
async function testCreateSession() {
    ...
    if (response.status === 201 && response.data?.sessionCode) {
        ...
        logSuccess(`Session created successfully with code: ${testState.sessionCode}`);
        logInfo(`Response: ${JSON.stringify(response.data)}`);
...
async function testGetSession() {
    ...
    const response = await makeRequest('GET', `/session/${testState.sessionCode}`);
    
    if (response.status === 200 && response.data?.code === testState.sessionCode) {
        testState.sessionData = response.data;
        logSuccess('Session retrieval successful');
        logInfo(`Session: ${response.data.name} (${response.data.participants.length}/${response.data.maxSize})`);
...
async function testJoinSession() {
    ...
    const userData = {
        userName: testState.testUser
    };
    
    const response = await makeRequest('POST', `/session/${testState.sessionCode}/join`, userData);
    ...
    if (response.status === 200) {
        ...
        // Test duplicate join
        const duplicateResponse = await makeRequest('POST', `/session/${testState.sessionCode}/join`, userData);
        ...
        const invalidResponse = await makeRequest('POST', '/session/INVALID/join', userData);
        ...
        const noNameResponse = await makeRequest('POST', `/session/${testState.sessionCode}/join`, {});
        if (noNameResponse.status === 400) {
            logSuccess('Correctly rejected join request without userName');
        } else {
            logWarning('Should reject join request without userName');
        }
...
async function testGetSessionParticipants() {
    ...
    const response = await makeRequest('GET', `/session/${testState.sessionCode}/participants`);
    ...
    if (response.status === 200 && Array.isArray(response.data)) {
        ...
        // Test with non-existent session
        const notFoundResponse = await makeRequest('GET', '/session/INVALID/participants');
        if (notFoundResponse.status === 404) {
            logSuccess('Correctly returned 404 for non-existent session participants');
        } else {
...
async function testLastfmTopArtists() {
    ...
    const response = await makeRequest('GET', `/lastfm/top-artists/${testState.lastfmTestUser}`);
    
    if (response.status === 200) {
        logSuccess(`Top artists for ${testState.lastfmTestUser} retrieved successfully`);
        ...
        logWarning(`Last.fm API might be unavailable or user '${testState.lastfmTestUser}' not found`);
        logInfo(`Response: ${JSON.stringify(response.data)}`);
...
async function testLastfmAuthInitiation() {
    ...
    if (response.status === 200 && response.data?.authUrl && response.data?.success) {
...
async function testLastfmAuthCallback() {
    ...
    const invalidTokenResponse = await makeRequest('GET', '/auth/lastfm/callback', null, { token: 'invalid_token' });
    
    if (invalidTokenResponse.status === 400 || invalidTokenResponse.status === 500) {
    ...
    logInfo('Callback testing completed (real token testing requires manual Last.fm auth flow)');
    return 'partial';
}
...
async function promptForManualTest() {
    ...
    return new Promise((resolve) => {
        ...
        console.log('1. Open browser to: http://localhost:3000/auth/lastfm');
        console.log('2. Complete the Last.fm authorization');
        console.log('3. Observer the callback handling');
        console.log('');
        
        rl.question('Do you want to perform this manual test now? (y/N): ', (answer) => {
            rl.close();
            if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                log('Please complete the manual test in your browser...', COLORS.CYAN);
                resolve(true);
            } else {
                resolve(false);
            }
...
async function testServerConnection() {
    ...
    try {
        ...
        logError('Make sure the backend server is running on http://localhost:3000');
...
async function runAllTests() {
    ...
    console.log(`${COLORS.BLUE}Testing all backend routes as specified in instructions.md${COLORS.RESET}`);
    ...
    for (const test of tests) {
        try {
            ...
            if (result === true) {
                ...
                results.partial++;
                logWarning(`${test.name}: Partial success (likely due to environment/config)`);
            } else {
                results.failed++;
    ...
    if (testState.sessionCode) {
        log(`\n💡 Test session code for reference: ${testState.sessionCode}`, COLORS.CYAN);
    }
    
    console.log('\n' + COLORS.BLUE + 'Routes tested according to instructions.md:' + COLORS.RESET);
    ...
    console.log('• GET /lastfm/top-artists/:user - Fetch Last.fm user top artists');
